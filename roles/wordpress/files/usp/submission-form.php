...
<div id="user-submitted-posts">
	...
	<form id="usp_form" method="post" enctype="multipart/form-data" action="">
		...
		<fieldset class="usp-name">
			<label for="user-submitted-name"><?php _e('Il tuo nickname su disqus', 'usp'); ?></label>
			<input name="user-submitted-name" type="text" value="" placeholder="<?php _e('Inseriscilo qui', 'usp'); ?>"<?php if (usp_check_required('usp_name')) echo $required; ?> class="usp-input">
		</fieldset>
		<?php } if (($usp_options['usp_url'] == 'show' || $usp_options['usp_url'] == 'optn') && ($usp_options['usp_use_url'] == false)) { ?>
		
		<fieldset class="usp-url">
			<label for="user-submitted-url"><?php _e('Il tuo sito (opzionale)', 'usp'); ?></label>
		...
		<fieldset class="usp-email">
			<label for="user-submitted-email"><?php _e('La tua email', 'usp'); ?></label>
		...
		<fieldset class="usp-title">
			<label for="user-submitted-title"><?php _e('Titolo articolo', 'usp'); ?></label>
			<input name="user-submitted-title" type="text" value="" placeholder="<?php _e('Inserisci qui il titolo', 'usp'); ?>"<?php if (usp_check_required('usp_title')) echo $required; ?> class="usp-input">
		</fieldset>
		<?php } if ($usp_options['usp_tags'] == 'show' || $usp_options['usp_tags'] == 'optn') { ?>
		
		<fieldset class="usp-tags">
			<label for="user-submitted-tags"><?php _e('Tag (separate da virgola)', 'usp'); ?></label>
			<input name="user-submitted-tags" type="text" value="" placeholder="<?php _e('Tag (min 3, obbligatorie se rapida)', 'usp'); ?>"<?php if (usp_check_required('usp_tags')) echo $required; ?> class="usp-input">
		</fieldset>
		<?php } if ($usp_options['usp_captcha'] == 'show') { ?>
		
		<fieldset class="usp-captcha">
			<label for="user-submitted-captcha"><?php echo $usp_options['usp_question']; ?></label>
			<input name="user-submitted-captcha" type="text" value="" placeholder="<?php _e('Rispondi se non sei un robot', 'usp'); ?>"<?php echo $required; ?> class="usp-input exclude<?php echo $captcha; ?>">
		</fieldset>
		<?php } { ?>
		
		<fieldset class="usp-quick">
			<label for="user-submitted-quick"><?php _e('Segnalazione rapida', 'usp'); ?></label>
			<input name="user-submitted-quick" type="checkbox" value="" class="usp-check">
		</fieldset>
		<?php } if (($usp_options['usp_category'] == 'show' || $usp_options['usp_category'] == 'optn') && ($usp_options['usp_use_cat'] == false)) { ?>
		
		<fieldset class="usp-category">
			<label for="user-submitted-category"><?php _e('Categoria del link', 'usp'); ?></label>
			<select name="user-submitted-category"<?php if (usp_check_required('usp_category')) echo $required; ?> class="usp-select">
				<option value=""><?php _e('Scegli la categoria migliore..', 'usp'); ?></option>
		...
		<fieldset class="usp-content">
			...
			<label for="user-submitted-content"><?php _e('Descrizione articolo', 'usp'); ?></label>
			<textarea name="user-submitted-content" rows="5" placeholder="<?php _e('Inserisci qui una descrizione del link segnalato. Più sarà dettagliata, più saremo contenti. Se prendi del testo dal pezzo, ti preghiamo di segnalarcelo', 'usp'); ?>"<?php if (usp_check_required('usp_content')) echo $required; ?> class="usp-textarea"></textarea>
			<?php } ?>
